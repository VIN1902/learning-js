...
# High level Design
1. Script starts → GEC created → memory creation phase → code execution phase  
2. Synchronous code runs → callstack executes functions one-by-one, each functions is used to create its own stack-frame by engine. → after FEC creation memory phase and execution phase take place for that too.
...
- Stack-frame consists of 1. function's parameters 2. local variables 3. info about return statement when function finishes.

- There's also something called Global Execution Context (GEC). its the first stack-frame that engine loads in callstack to run the script. even if there's no function declared or used in the script. The GEC is assigned to 'this'.

- When a function is done executing its stack-frame is popped, along with whatever it had. similar case in GEC. LIFO mechanism to pop.
- Two examples:
    1. There were 2 different functions. GEC created -> 1st FEC creates on top of GEC and pops -> 2nd FEC creates on top of GEC and pops. -> GEC pops.
    2. 2 functions but 1st calls 2nd one inside itself. GEC created -> 1st FEC creates on top of GEC ->  2nd FEC creates on top 1st FEC then pops -> 1st FEC pops -> GEC pops.

- Local variables are scoped to stack-frame of their respective functions.
...
- There's also two phases 1. memory phase 2. code phase, which happens every time a new execution context is created for either GEC or FEC.
    - memory phase only the memory is loaded with variable-names assigned undefined and function-names assigned their definition. (hoisting)
    - code phase then line-by-line goes over script and assigns the variables with actual values if defined and any computation.
...
- Inside each EC, the engine also creates a Lexical Environment (LE). Think of this as the “variable storage + scope rules” attached to that stack-frame.
    - LE is a data stucture and upon its creation it defines the 'scope' for that EC.
    - Scope: The scope of a variable or function is the region of the code where that variable or function can be accessed. This region is defined by the LE in which it is created.
- An LE has two parts:
...
- `var` was original and at that time only FEC/GEC's LE defined a 'scope'. This lead to `var` being accessible outside of if-else/loops made in GEC. Cause everything is still technically within GEC.
- `let` and `const` came later and created their own new LE inside the FEC/GEC they were written in. This new LE which is alongside the original LE of that EC, defines its own new scope called 'block scope'. This prevents them being accessed outside of it.
- So inside 1 EC we have the LE defining the overall scope (used by `var`) and then we have another LE defining block scope (used by `let` and `const`).
